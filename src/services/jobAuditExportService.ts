...
export async function exportWeeklyJobAuditToPDF(
    ...
    try {
        ...
        drawKpiCard(pdf, 15, kpiStartY, 60, 30, 'Ocupación Dept.', `${globalStats.occupancy.toFixed(1)}%`, `${globalStats.totalCovered.toFixed(1)}h / ${globalStats.totalPresence.toFixed(1)}h`, [79, 70, 229]);
        ...
        if (includeEmployeeDetails && employeeData.length > 0) {
            ...
            autoTable(pdf, {
                startY: yPosition,
                head: [['ID', 'Operario', 'Presencia', 'Cubierto', 'Fuga', 'Ocupación']],
                body: tableData,
                theme: 'striped',
                headStyles: {
                    fillColor: [79, 70, 229],
